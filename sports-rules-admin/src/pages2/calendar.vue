<template>
  <q-page>
    <q-toolbar>
      ...
      <q-select outlined dense emit-value map-options label="Change theme" v-model="theme" :options="themesList" class="float-right" :class="$q.dark.isActive?'bg-black':'bg-white'" style="width:200px"></q-select>
    </q-toolbar>
    <div style="width: 100%;" class="bg-white">
      <q-calendar v-model="selectedDate" :view="view_name" ref="calendar" locale="en-us" enable-theme :theme="theme" style="height: 550px;border:1px solid #e0e0e0">
        <template #day="{ date }">
          <template v-for="(event, index) in getEvents(date)">
            <q-badge :key="index" style="width: 100%; cursor: pointer; height: 16px; max-height: 16px" class="q-event" :class="badgeClasses(event, 'day')" :style="badgeStyles(event, 'day')">
