  ...
  <q-page class="q-pt-xs">
    <div class="q-ma-md">
      <div class="row q-my-md shadow" style="border-radius: 4px;border: 1px solid rgba(0, 0, 0, 0.12);">
        ...
        <div class="col-md-3 col-lg-3 col-sm-12 col-xs-12 box_2" style="border-left: 1px solid #efefef;" v-bind:class="{'border-top': $q.platform.is.mobile}">
          <q-card class="no-shadow q-pa-sm">
            <q-item class="q-pb-none q-pt-xs">
              <q-item-section>
        ...
        <div class="col-md-3 col-lg-3 col-sm-12 col-xs-12 box_3" style="border-left: 1px solid #efefef;" v-bind:class="{'border-top': $q.platform.is.mobile}">
          <q-card class="no-shadow q-pa-sm">
            <q-item class="q-pb-none q-pt-xs">
              <q-item-section>
        ...
        <div class="col-md-3 col-lg-3 col-sm-12 col-xs-12 box_4" style="border-left: 1px solid #efefef;" v-bind:class="{'border-top': $q.platform.is.mobile}">
          <q-card class="no-shadow q-pa-sm">
            <q-item class="q-pb-none q-pt-xs">
              <q-item-section>
          ...
          <q-card flat bordered class="shadow q-pa-none q-ma-none">
            <q-card-section class="row">
              <div :class="!$q.dark.isActive? 'text-grey-7':'text-white'" class="text-h6 col-12">Sales vs Goals
                <q-btn flat dense icon="fas fa-download" class="float-right" @click="SaveImage('sales_goals')" :color="!$q.dark.isActive? 'grey-8':'white'">
          ...
          <q-card flat bordered class="shadow q-pa-none q-ma-none">
            <q-card-section class="row">
              <div :class="!$q.dark.isActive? 'text-grey-7':'text-white'" class="text-h6 col-12">Market Share & Growth
                <q-btn flat dense icon="fas fa-download" class="float-right" @click="SaveImage('market_share')" :color="!$q.dark.isActive? 'grey-8':'white'">
                  <q-tooltip>Download</q-tooltip>
                </q-btn>
              </div>
            </q-card-section>
          ...
          <q-card flat bordered class="shadow q-pa-none q-ma-none">
            <q-card-section>
              <div :class="!$q.dark.isActive? 'text-grey-7':'text-white'" class="text-h6">Key Competitors
                <q-btn flat dense icon="fas fa-download" class="float-right" @click="SaveImage('key_competitors')" :color="!$q.dark.isActive? 'grey-8':'white'">
                  <q-tooltip>Download</q-tooltip>
                </q-btn>
              </div>
            </q-card-section>
          ...
          <q-card flat bordered class="shadow q-pa-none q-ma-none">
            <q-card-section>
              <div :class="!$q.dark.isActive? 'text-grey-7':'text-white'" class="text-h6">Sales Pipeline by Sales Rep
                <q-btn flat dense icon="fas fa-download" class="float-right" @click="SaveImage('sales_pipeline')" :color="!$q.dark.isActive? 'grey-8':'white'">
                  <q-tooltip>Download</q-tooltip>
                </q-btn>
              </div>
            </q-card-section>
      ...
      <div class="row q-col-gutter-md q-pt-md">
        <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
          <q-card flat bordered class="shadow">
            <q-table title="All Activities" :data="data" :hide-header="mode === 'grid'" :table-class="$q.dark.isActive?'text-white':'text-grey-8'" :table-header-class="$q.dark.isActive?'text-white':'text-grey-9'" :columns="columns" row-key="name" :grid="mode=='grid'" :filter="filter" :pagination.sync="pagination">
              <template v-slot:top-left="props">
                <div :class="$q.dark.isActive?'text-white':'text-grey-7'" class="text-h6">All Activities</div>
              </template>
              <template v-slot:body-cell-status="props">
                <q-td :props="props" class="text-center">
                  <q-chip v-bind:class="{'chip_completed': props.row.status == 'Completed', 'chip_pending': props.row.status == 'Pending'}" text-color="white" dense class="text-weight-bolder text-center" square style="width: 85px"><span class="full-width inline-block flex flex-center text-center">{{props.row.status}}</span>
                  </q-chip>
                </q-td>
              </template>
              <template v-slot:top-right="props">
                ...
                <q-btn flat round dense :icon="props.inFullscreen ? 'fullscreen_exit' : 'fullscreen'" @click="props.toggleFullscreen" v-if="mode === 'list'" class="text-grey-7">
                  <q-tooltip :disable="$q.platform.is.mobile" v-close-popup>{{props.inFullscreen ? 'Exit Fullscreen' : 'Toggle Fullscreen'}}
                  </q-tooltip>
                </q-btn>

                <q-btn flat round dense :icon="mode === 'grid' ? 'list' : 'grid_on'" @click="mode = mode === 'grid' ? 'list' : 'grid'; separator = mode === 'grid' ? 'none' : 'horizontal'" v-if="!props.inFullscreen" class="text-grey-7">
                  <q-tooltip :disable="$q.platform.is.mobile" v-close-popup>{{mode==='grid' ? 'List' : 'Grid'}}
                  </q-tooltip>
                </q-btn>

                <q-btn style="background-color:#37CDE3;color:white" icon-right="archive" label="Export to csv" no-caps @click="exportTable" />
              </template>
            </q-table>
          </q-card>
        </div>
